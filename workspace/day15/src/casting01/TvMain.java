...
//		tv.neflex(); 부모 객체의 값은 자식 클래스에 있는 멤버 사용 불가능
		tv.powerOnOff();
		
		SmartTv smartTv = new SmartTv();
		System.out.println(smartTv);
		smartTv.neflex(); // 자식 객체의 값은 자식 클래스에 있는 멤버 사용 가능
		smartTv.chUp(); // 자식 객체의 값은 부모 클래스에 있는 멤버도 사용 가능
		
		//Up casting
		//방법 1
		//SmartTv()는 Tv 클래스의 참조 변수 tv1에 할당된다
		//부모클래스 타입의 참조 변수로 자식 클래스 타입에 참조값을 대입하는 것이다
		Tv tv1 = new SmartTv(); //부모 타입 객체에 자식의 참조값을 대입
...
//		방법2
		Tv tv2 =(Tv)smartTv;
		//tv2는 SmartTv 클래스의 객체를 참조하지만 업캐스팅으로 인해 Tv타입으로 취급된다
//		따라서 Tv 클래스의 필드와 메소드에만 접근 할 수 있다
